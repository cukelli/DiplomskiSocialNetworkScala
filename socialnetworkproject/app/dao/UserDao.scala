...
class UsersTable(tag: Tag) extends Table[User](tag, "Users") {
  ...
  def * = (userID, email, lastName, firstName, password, gender, profilePicture) <>
    (User.tupled, User.unapply)

  implicit val genderMapper: BaseColumnType[Genders.Gender] = MappedColumnType.base[Genders.Gender, String](
    e => e.toString,
...
class UserDao @Inject()(
  ...
  def getById(userID: Long): Future[Option[User]] = db.run(UsersTableQuery.filter(_.userID === userID).result.headOption)
  ...
  def  getByEmailAndPassword(email: String, password: String): Future[Option[User]] = {
    db.run(Users.filter(_.email === email).result.map(_.headOption.filter(user =>
  ...
  def update(user: User): Future[Unit] = db.run(UsersTableQuery.filter(_.userID === user.UserID).update(user)).map(_ => ())
  def delete(userID: Long): Future[Unit] = db.run(UsersTableQuery.filter(_.userID === userID).delete).map(_ => ())
  ...
  def updatePassword(userId: Long, oldPassword: String, newPassword: String): Future[Boolean] = {
